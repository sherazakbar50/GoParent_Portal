...
export class CaseNoteService extends ApiHandler {
  ...
  private caseNoteSubject$: BehaviorSubject<CaseNoteDto[]> = new BehaviorSubject<CaseNoteDto[]>(
    undefined,
  )
  ...
  async getCasesNote(Id: number) {
    let response = await this.Get(Id, API_URL + API_ENDPOINTS.GetCaseNoteByCase)
  ...
  addUpdate(caseNote: CaseNoteDto) {
    return this.Post(undefined, API_URL + API_ENDPOINTS.AddUpdateCaseNote, caseNote)
      .pipe(map(x => x.ResponseData))
      .toPromise<boolean>()
  }

  DeleteCaseNote(id) {
    return this.Post(id, API_URL + API_ENDPOINTS.deleteCaseNote, undefined)
      .pipe(map(x => x.ResponseData))
      .toPromise()
  }
}
