...
export class FamilyDocumentsService extends ApiHandler {
  ...
  private documentsSubject$: BehaviorSubject<FamilyDocumentsDto[]> = new BehaviorSubject<FamilyDocumentsDto[]>(undefined);
  ...
  add(documentElement: FormData) {
    ...
    return this.Post(undefined, API_URL + API_ENDPOINTS.AddUpdateDocument, documentElement).
      pipe(map(x => x.ResponseData)).toPromise();
  }


  async getFamilyDocuments() {
    let response = await this.GetAll(API_URL + API_ENDPOINTS.getFamilyDocuments).pipe(map(x => x.ResponseData)).toPromise();
    if (response) {
