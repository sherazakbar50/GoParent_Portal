...
export class EventsService extends ApiHandler {
    ...
    saveEvent(data: any) {
        return this.Post(0, API_URL + API_ENDPOINTS.SaveFamilyEvent,data).pipe(map(x=>x.ResponseData)).toPromise();
    }

    getEvents(date:Date,hello:number){
        this.GetAll(API_URL + API_ENDPOINTS.GetAllFamilyEvents).subscribe(res => {
            if(res.IsSuccessful)
                this.eventSubject$.next(res.ResponseData);
        });
    }
    getMonthWiseEvent(date:Date){
        this.GetAll(API_URL + API_ENDPOINTS.GetMonthWiseFamilyEvents,`date=${date.toUTCString()}`).subscribe(res => {
            if(res.IsSuccessful)
                this.eventSubject$.next(res.ResponseData);
        });
    }
    ...
    deleteEvent(eventId:number) {
        return this.Delete(API_URL + API_ENDPOINTS.DeleteEvent,eventId).pipe(map(x=>x.ResponseData)).toPromise();
    }
}  