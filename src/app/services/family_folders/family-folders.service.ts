...
export class FamilyFoldersService extends ApiHandler {
  ...
  private folderSubject$: BehaviorSubject<FolderElementdto[]> = new BehaviorSubject<
    FolderElementdto[]
  >(undefined)
  ...
  add(fileElement: FolderElementdto) {
    return this.Post(undefined, API_URL + API_ENDPOINTS.AddUpdateFolder, fileElement)
      .pipe(map(x => x.ResponseData))
      .toPromise<boolean>()
  }
  async getFolders(caseId?: number) {
    let response = await this.Get(caseId, API_URL + API_ENDPOINTS.getAllFolders)
      .pipe(map(x => x.ResponseData))
      .toPromise()
    if (response) {
  ...
  update(fileElement: FolderElementdto) {
    return this.Post(fileElement.FolderId, API_URL + API_ENDPOINTS.AddUpdateFolder, fileElement)
      .pipe(map(x => x.ResponseData))
      .toPromise<boolean>()
  }
